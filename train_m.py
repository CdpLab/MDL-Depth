...
class Trainer:
    def __init__(self, options):
        ...
        datasets_dict = {"kitti": datasets.KITTI_MDL_Dataset,
                         "kitti_odom": datasets.KITTIOdomDataset,
                         "cityscapes": datasets.Cityscapes_MDL_Dataset,
        ...
        # create models
        self.models["MDL"] = GMNet(scale=self.opt.mdl_scale)
        self.models["MDL"].to(self.device)
        if self.opt.pretrained_path:
            logging.info("Loading pretrained model from folder: {}".format(self.opt.pretrained_path))
            try:
                self.models["MDL"].load_state_dict(torch.load(self.opt.pretrained_path)["MDL"])
            except:
                self.models["MDL"].load_state_dict(torch.load(self.opt.pretrained_path))
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            img1_pred, loss_mdl, flow0, flow1, _ = self.models['MDL'](img0, img2, embt, img1)
           
            start_bp_time = time.time()
            self.model_optimizer.zero_grad()

            losses = {}
            losses["loss"] = loss_mdl
    ...
    def save_model(self, ep_end=False, batch_idx=0):
        ...
        to_save['mdl_scale'] = self.opt.mdl_scale 
