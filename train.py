...
class Trainer:
    def __init__(self, options):
        ...
        # create models
        if self.opt.backbone == "LSM":
            self.models["encoder"] = LSM.DepthEncoder(model='lsm',
                                            drop_path_rate=0.2,
                                            width=self.opt.width, height=self.opt.height)
            model_dict = self.models["encoder"].state_dict()
            pretrained_dict = torch.load("./weights/lsm-pretrain.pth")['model']
            ...
            self.models["depth"] = LSM.DepthDecoder(
        ...
        self.model_mdl_train = GMNet(scale="large")
        self.model_mdl_test = GMNet(scale="small")
        if self.opt.dataset == "kitti":
            self.model_mdl_train.load_state_dict(torch.load("./weights/GMNet_L_KITTI.pth")["MDL"])
            self.model_mdl_test.load_state_dict(torch.load("./weights/GMNet_S_KITTI.pth")["MDL"])
        elif self.opt.dataset == "cityscapes":
            self.model_mdl_train.load_state_dict(torch.load("./weights/GMNet_L_CS.pth")["MDL"])
            self.model_mdl_test.load_state_dict(torch.load("./weights/GMNet_S_CS.pth")["MDL"])
        else:
            pass
        self.model_mdl_train.to(self.device).eval()
        self.model_mdl_test.to(self.device).eval()

        if self.opt.world_size > 1:
            self.model_mdl_train = nn.SyncBatchNorm.convert_sync_batchnorm(self.model_mdl_train)
            self.model_mdl_train = nn.parallel.DistributedDataParallel(self.model_mdl_train, device_ids=[self.opt.local_rank], output_device=self.opt.local_rank, find_unused_parameters=True)
            self.model_mdl_test = nn.SyncBatchNorm.convert_sync_batchnorm(self.model_mdl_test)
            self.model_mdl_test = nn.parallel.DistributedDataParallel(self.model_mdl_test, device_ids=[self.opt.local_rank], output_device=self.opt.local_rank, find_unused_parameters=True)
    ...
    def test_cityscapes_mf(self):
        ...
        for idx, data in enumerate(self.test_loader):
            ...
            flow_0_n1, flow_0_p1, merge_mask_01 = self.model_mdl_test(img_n1, img_p1, embt, onlyFlow=True)
    ...
    def process_batch(self, inputs):
        ...
        with torch.no_grad():
            img_nt, flow_nt_n1, flow_nt_0, merge_mask_nt = self.model_mdl_train(img_n1, img_0, embt)
            img_pt, flow_pt_0, flow_pt_p1, merge_mask_pt = self.model_mdl_train(img_0, img_p1, embt)
            flow_0_n1, flow_0_p1, merge_mask_01 = self.model_mdl_train(img_n1, img_p1, embt, onlyFlow=True)     
