...
def eval_args():
    ...
    parser.add_argument("--mdl_scale",
                        type=str,
                        help="the scale of GMNet",
                        default="small",
                        choices=["large", "small"])
...
def load_model(args):
    ...
    if args.backbone == "LSM":
        depth_encoder = LSM.DepthEncoder(model='lsm', drop_path_rate=0.2,
                        width=args.width, height=args.height)
        depth_decoder = LSM.DepthDecoder(depth_encoder.num_ch_enc, range(1))
    ...
    model_mdl = GMNet(scale=args.mdl_scale)
    if args.training_data == "kitti":
        if args.mdl_scale == "large":
            model_mdl.load_state_dict(torch.load("./weights/GMNet_L_KITTI.pth")["MDL"])
        if args.mdl_scale == "small":
            model_mdl.load_state_dict(torch.load("./weights/GMNet_S_KITTI.pth")["MDL"])
    if args.training_data == "cityscapes":
        if args.mdl_scale == "large":
            model_mdl.load_state_dict(torch.load("./weights/GMNet_L_CS.pth")["MDL"])
        if args.mdl_scale == "small":
            model_mdl.load_state_dict(torch.load("./weights/GMNet_S_CS.pth")["MDL"])
    model_mdl.cuda().eval()
    ...
    flow_0_n1, flow_0_p1, merge_mask_01 = model_mdl(img_n1, img_p1, embt, onlyFlow=True)  
    flops_mdl, params_mdl = profile(model_mdl, inputs=(img_n1, img_p1, embt, None, (1.0, 0.5), True,), verbose=False)   
    ...
    params = params_mdl + params_e + params_fm + params_d
    flops_single = flops_mdl + 3*flops_e + flops_fm + flops_d
    flops_average = flops_mdl + flops_e + flops_fm + flops_d
    ...
    return model_mdl, depth_encoder, depth_decoder, fusion_module


def test_kitti_mf(args, dataloader, model_mdl, depth_encoder, depth_decoder, fusion_module, eval_split='eigen'):
    MIN_DEPTH = 1e-3
    MAX_DEPTH = 80
...
def test_cityscapes_mf(args, model_mdl, dataloader, depth_encoder, depth_decoder, fusion_module):
    MIN_DEPTH = 1e-3
    MAX_DEPTH = 80
...
def main(args):
    ...
    model_mdl, depth_encoder, depth_decoder, fusion_module = load_model(args)
    ...
    if args.kitti_path:
        ...
        with torch.no_grad():
            test_kitti_mf(args, dataloader, model_mdl, depth_encoder, depth_decoder, fusion_module, "eigen")
        ...
        dataloader = DataLoader(dataset, args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=False, drop_last=False)
        with torch.no_grad():
            test_kitti_mf(args, dataloader, model_mdl, depth_encoder, depth_decoder, fusion_module, "eigen_benchmark")
    ...
    if args.cityscapes_path:
        ...
        dataloader = DataLoader(dataset, args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=False, drop_last=False)
        with torch.no_grad():
            test_cityscapes_mf(args, model_mdl, dataloader, depth_encoder, depth_decoder, fusion_module)  
