...
class PositionalEncodingFourier(nn.Module):
    ...
    def __init__(self, hidden_dim=32, dim=768, temperature=10000):
        ...
        self.pos_weight = nn.Parameter(torch.ones(1, 1, 1, 1))
        self.pos_bias = nn.Parameter(torch.zeros(1, 1, 1, 1))
    ...
    def forward(self, B, H, W):
        ...
        pos = pos * self.pos_weight + self.pos_bias
...
class DeformableDilatedConv(nn.Module):
    def __init__(self, dim, k, dilation=1, stride=1):
        super().__init__()
...
class DilatedConv(nn.Module):
    def __init__(self, dim, k, dilation=1, stride=1, drop_path=0.,
                 layer_scale_init_value=1e-6, expan_ratio=6, use_deformable=False):
        super().__init__()
        
        if use_deformable:
            self.ddwconv = DeformableDilatedConv(dim, k, dilation, stride)
        else:
...
class StripPool(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.sp_h = nn.AdaptiveAvgPool2d((None, 1))  
        self.sp_w = nn.AdaptiveAvgPool2d((1, None))  
    ...
    def forward(self, x):
        ...
        horizontal = self.sp_h(x)
        horizontal = F.interpolate(horizontal, (h, w), mode='nearest')
        
        vertical = self.sp_w(x)
        vertical = F.interpolate(vertical, (h, w), mode='nearest')
        
        combined = identity + horizontal + vertical
        return self.conv(combined)


class Light(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
                 use_pos_emb=True, num_heads=6, qkv_bias=True, attn_drop=0., drop=0., use_strip_pool=True):
        super().__init__()

        self.dim = dim

        self.strip_pool = StripPool(dim) if use_strip_pool else nn.Identity()
        self.pos_embd = None
        if use_pos_emb:
        ...
        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = LinearAttention(self.dim, num_heads=num_heads, proj_drop=drop)

        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  
            nn.Conv2d(dim, max(dim//16, 4), 1),  
            nn.GELU(),
            nn.Conv2d(max(dim//16, 4), dim, 1),  
            nn.Sigmoid()  
        )
...
class Light(nn.Module):
    def __init__(self, in_chans=3, model='light', height=192, width=640,
                 global_block=[1, 1, 1], global_block_type=['Light', 'Light', 'Light'],
        ...
        # Add more model configurations like 'light-s', 'light-t', 'light-m' etc. here...
        
        for g in global_block_type:
            assert g in ['None', 'Light']
        ...
        for i in range(3):
            ...
            for j in range(self.depth[i]):
                ...
                if j > self.depth[i] - global_block[i] - 1:
                    if global_block_type[i] == 'Light':
                        stage_blocks.append(Light(dim=self.dims[i], drop_path=dp_rates[cur + j],
                                                 expan_ratio=expan_ratio,
                                                 use_pos_emb=use_pos_embd_xca[i], num_heads=heads[i],
                                                 layer_scale_init_value=layer_scale_init_value,use_strip_pool=True))
                    else:
